...
namespace Valve.VR.InteractionSystem
{
	...
	public class Movable : MonoBehaviour
	{
        ...
        public ReleaseStyleMovable releaseVelocityStyle = ReleaseStyleMovable.GetFromHand;
        ...
        protected virtual void OnDetachedFromHand(Hand hand)
        {
            ...
            ResetPiece reset = GetComponent<ResetPiece>();
            if(reset != null)
            {
                reset.CheckResetPos();
            }
        ...
        {
            if (hand.noSteamVRFallbackCamera && releaseVelocityStyle != ReleaseStyleMovable.NoChange)
                releaseVelocityStyle = ReleaseStyleMovable.ShortEstimation; // only type that works with fallback hand is short estimation.

            switch (releaseVelocityStyle)
            {
                case ReleaseStyleMovable.ShortEstimation:
                    velocityEstimator.FinishEstimatingVelocity();
                    velocity = velocityEstimator.GetVelocityEstimate();
                    angularVelocity = velocityEstimator.GetAngularVelocityEstimate();
                    break;
                case ReleaseStyleMovable.AdvancedEstimation:
                    hand.GetEstimatedPeakVelocities(out velocity, out angularVelocity);
                    break;
                case ReleaseStyleMovable.GetFromHand:
                    velocity = hand.GetTrackedObjectVelocity(releaseVelocityTimeOffset);
                    angularVelocity = hand.GetTrackedObjectAngularVelocity(releaseVelocityTimeOffset);
                    break;
                default:
                case ReleaseStyleMovable.NoChange:
                    velocity = rigidbody.velocity;
                    angularVelocity = rigidbody.angularVelocity;
                    break;
            }

            if (releaseVelocityStyle != ReleaseStyleMovable.NoChange)
    ...
    public enum ReleaseStyleMovable
    {
        NoChange,
        GetFromHand,
        ShortEstimation,
